...
import org.dreamscale.springboot.validation.ValidationErrorCodes
...
class RestResponseEntityExceptionHandlerSpec extends Specification {
    ...
    def "should respond with ERROR_ENTITY_DATA_FORMAT if an enum cannot be deserialized"() {
        ...
        then:
        def exception = thrown(HttpResponseException)
        assertResponse(exception, ValidationErrorCodes.ERROR_ENTITY_DATA_FORMAT, HttpStatus.SC_BAD_REQUEST, "Cannot deserialize value of type `org.dreamscale.springboot.exception.ValidatableEnum`")
    }
